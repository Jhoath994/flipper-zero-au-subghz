...
- Kelly, the repo maintainers, and every contributor take zero responsibility for you being a muppet.
...
def generate_prep_notes(
    ...
    lines.append("=" * 72)
    lines.append("KELLY'S ROLLJAM PREP NOTES — FOR YOUR OWN HARDWARE ONLY")
    lines.append("=" * 72)
    lines.append("")
    ...
    lines.append("The repo maintainers and Kelly accept ZERO responsibility.")
    lines.append("")
    ...
    lines.append("Generated by Kelly's rolljam_prep.py — file analysis only.")
...
def main():
    parser = argparse.ArgumentParser(
        description="Kelly's RollJam Prep — timing analysis + notes generator for your own .sub captures only. READ docs/rolljam_reality.md FIRST."
    )
    ...
    print("\n[Kelly] RollJam Prep starting. Heavy legal warnings are in the output file and in docs/rolljam_reality.md")
    print("[Kelly] This tool does NOTHING on the radio. It is notes and timing stats only.\n")
    ...
    if len(files) < 2:
        print("[Kelly] You need at least TWO .sub RAW captures for meaningful prep notes.")
        print("        Run with --captures file1.sub file2.sub  or  --dir some_folder/")
        return

    print(f"[Kelly] Analysing {len(files)} capture files...")
    ...
    print("[Kelly] Open it. Read the warnings again. Then decide if this path is even worth it on your own car.")
    print("[Kelly] Most of the time the honest answer is: better captures + distance technique will teach you more.")
